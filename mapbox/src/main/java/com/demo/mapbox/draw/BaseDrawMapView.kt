...
class BaseDrawMapView @JvmOverloads constructor(
    ...
    private var mLoadDrawMapViewListener: LoadDrawMapViewListener? = null
    ...
    fun initDrawMapView() {
        ...
        baseMapView?.setLoadMapListener(object : LoadMapListener {
            ...
            override fun onLoadComplete(mapboxMap: MapboxMap?) {
                drawLandManager?.addTouchListener()
                drawLandManager?.addMapClickListener()
                drawLandManager?.addMapLongClickListener()
                if (mLoadDrawMapViewListener != null) {
                    mLoadDrawMapViewListener?.loadSuccess(mapboxMap)
                }
            }
    ...
    fun setOnLoadDrawMapViewListener(loadDrawMapViewListener: LoadDrawMapViewListener?) {
        mLoadDrawMapViewListener = loadDrawMapViewListener
    }
    ...
    override fun startDraw(entity:com.demo.drawland.LandEntity?,drawType: DrawType?) {
        if (drawLandManager != null) {
            drawLandManager?.startDraw(drawType,entity)
        }
        if (mLandView != null) {
            mLandView?.setDrawType(drawType)
            mMirrorMapViewFront?.mirrorLandView?.setDrawType(drawType)
