 ...
 * @desc 镜像勾画内容view
 */
class MirrorLandView @JvmOverloads constructor(
    context: Context?,
    ...
    private fun drawPoint(canvas: Canvas) {
        for (i in mPoints!!.indices) {
            ...
            canvas.drawCircle(
                pointF!!.x,
                pointF.y,
                DensityUtil.dip2px(12f).toFloat(),
                mOuterPointPaint
            )
            if (i == mSelectIndex) {
                canvas.drawCircle(
    ...
    fun setTouchEvent(event: MotionEvent) {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                mPoints?.removeAt(mSelectIndex)
                mPoints?.add(mSelectIndex, pointF)
                setPoints(mPoints, mSelectIndex)
            }
